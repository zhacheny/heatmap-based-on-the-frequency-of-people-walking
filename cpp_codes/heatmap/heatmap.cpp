...
unsigned char *colorize(struct info *inf, unsigned char* pixels_bw, unsigned char* scheme, unsigned char* pixels_color, 
              int opacity)
{
    int width = inf->width;
    int height = inf->height;
                  ...
                  float minX, float minY, float maxX, float maxY)
{
    ...
    if (NULL == points || NULL == pix_color ||
        w <= 0 || h <= 0 || cPoints <= 1 || opacity < 0 || dotsize <= 0)
    {
        fprintf(stderr, "Invalid parameter; aborting.\n");
        return NULL;
    }
	if (scheme_mode < M_CLASSIC)
		scheme_mode = M_CLASSIC;
	else if (scheme_mode > M_PGAITCH)
		scheme_mode = M_PGAITCH;

	uint8_t* scheme_data = static_cast<uint8_t*>( static_cast<void*>(const_cast<RGB*>(schemes[scheme_mode])));
    struct info inf = {0};
    ...
    pix_color = colorize(&inf, pixels_bw, scheme_data, pix_color, opacity);
